return {
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      -- Search For Directories including hidden
      vim.keymap.set('n', '<leader>sDh', function()
        ...
        pickers
          .new({}, {
            prompt_title = 'Find Directories (Include Hidden)',
            finder = finders.new_oneshot_job({ 'fd', '--type', 'd', '--hidden', '--no-ignore' }, {
              entry_maker = function(line)
                return setmetatable({ line, cwd }, mt_dir_entry)
              end,
            }),
          ...
          :find()
      end, { desc = '[s]earch [H]idden directories' })
